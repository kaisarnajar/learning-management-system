...
export default async function AdminFatwaDetailPage({
  ...
  return (
    <div>
      ...
      <ActionToast trigger={query.saved === "1"} paramName="saved" message={`Answer published.${query.email === "anonymous" ? " No notification email was triggered or sent for this anonymous submission." : query.email === "failed" ? " The notification email could not be sent — check SMTP settings in your environment." : query.email === "skipped" ? " SMTP is not configured, so no email was sent." : " The asker has been notified by email. If they don't see it, ask them to check their Spam/Junk folder."}`} variant="info" />
