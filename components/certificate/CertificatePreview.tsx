...
export function CertificatePreview({ enrollmentId }: { enrollmentId: string }) {
  ...
  const handleSendEmail = () => {
    startTransition(async () => {
      const result = await sendCertificateToEmailAction(enrollmentId);
      if (result.error) {
        addToast(result.error, "error");
      } else {
        addToast("Certificate sent successfully. Please check student's Inbox or Spam/Junk folder.", "success");
      }
    });
  };
      ...
      <div className="relative flex min-h-[600px] w-full flex-col items-center justify-center rounded-2xl border border-border bg-surface shadow-sm overflow-hidden p-4 md:p-8">
        ...
        {isLoading && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-surface/80 backdrop-blur-sm">
            ...
            <p className="text-sm font-medium text-muted">Generating Certificate PDF...</p>
          </div>
        )}
        ...
        <div className="relative w-full max-w-[850px] aspect-[1.414] bg-white rounded-xl overflow-hidden shadow-md ring-1 ring-border/50">
          <iframe
            ...
            title="Student Certificate Preview"
          />
        </div>
        
        <p className="mt-6 text-sm text-muted text-center max-w-lg">
          This is a preview of the official Student Certificate. You can email the high-quality PDF version directly to the student using the button above.
        </p>
