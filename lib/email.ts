...
// ---------------------------------------------------------------------------
// Email: Attendance Card with PDF attachment
// ---------------------------------------------------------------------------
...
export async function sendAttendanceCardEmail(params: AttendanceCardEmailParams): Promise<EmailSendResult> {
  ...
  const bodyHtml = `
    <p>Assalamu Alaikum <strong>${escapeHtml(displayName)}</strong>,</p>
    <p>Your academic attendance report (Attendance Card) for the course <strong>${escapeHtml(courseTitle)}</strong> has been generated.</p>
    <div style="margin:24px 0;padding:16px 20px;background:#f0f9ff;border-left:4px solid #0284c7;border-radius:0 6px 6px 0;">
      <p style="margin:0 0 6px 0;font-size:13px;font-weight:600;color:#0369a1;text-transform:uppercase;letter-spacing:0.5px;">Attendance Card Issued</p>
      <p style="margin:0;font-size:14px;">Your official attendance record has been prepared and generated successfully.</p>
    </div>
    <p>The PDF document is <strong>attached to this email</strong>. Please download and keep it for your records.</p>
    <p style="font-size:13px;color:#6b7280;">If you have any questions, please reply to this email.</p>`;
